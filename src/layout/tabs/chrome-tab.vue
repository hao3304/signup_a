  ...
  <div class="vue-tabs-chrome" :class="theme ? 'theme-' + theme : ''">
    <div class="tabs-content" ref="content">
      <div
        ...
        :key="i"
        :style="{
          left: (tabWidth - gap * 2) * i + gap + 'px',
          zIndex: i == tabs.length ? 0 : 1
        }"
      ></div>
      <div
        class="tabs-item"
      ...
      >
        ...
        <div class="tabs-main" :title="tab | tabLabelText(tabLabel)">
          ...
          <span class="tabs-label">{{ tab | tabLabelText(tabLabel) }}</span>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    addInstance(tab, i) {
      ...
      tab._instance.on("pointerDown", (e, pointer) =>
        this.handlePointerDown(e, tab, i)
      );
      tab._instance.on("dragMove", (e, pointer, moveVector) =>
        this.handleDragMove(e, moveVector, tab, i)
      );
      tab._instance.on("dragEnd", (e, pointer) =>
        this.handleDragEnd(e, tab, i)
      );
      tab._instance.on("staticClick", (e, pointer) =>
        this.handleClick(e, tab, i)
      );
    },
    addTab(...tabs) {
    ...
    handleDragMove(e, moveVector, tab, i) {
      ...
      let { instance, targetTab } = getInstanceAt(
        tabs,
        tab,
        tabWidth,
        tabKey,
